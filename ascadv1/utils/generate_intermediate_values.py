...
def save_real_values(trs = None,n_traces = None,save_file = True,plaintexts  = None,ciphertexts = None,keys = None,random = None):
    ...
    # Pour chaque trace, le texte en clair est converti en entier. 
    # Une instance de l'objet AES est créée avec la clé correspondante (fixe ou variable). 
    # Le texte en clair est converti en matrice d'état et aplati pour être stocké.
    for k in range(n_traces) :
        ...
        # La clé de début est aplanie et stockée. Si des masques sont utilisés, les clés masquées sont calculées et ajoutées à real_values. 
        # La clé de début est ensuite ajoutée à l'état de chiffrement.
        ...
        # Pour chaque tour de 1 à 10, l'état de chiffrement est transformé et les valeurs intermédiaires sont stockées. 
        # Les étapes incluent la substitution des bytes, le décalage des lignes, la mixage des colonnes (sauf au dernier tour) et 
        # l'ajout de la clé de tour. Les valeurs intermédiaires masquées sont également calculées et stockées si les masques sont utilisés.
